...
type TimelineButtonProps = {
  active?: boolean;          // on le garde pour plus tard
  onClick?: () => void;
};

export function TimelineButton({ active, onClick }: TimelineButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs shadow-[0_10px_28px_rgba(15,23,42,0.12)] backdrop-blur-sm
        ${
          active
            ? "border-indigo-400 bg-indigo-500/90 text-white dark:border-indigo-300 dark:bg-indigo-500/90 dark:text-white"
            : "border-slate-200 bg-white/90 text-slate-900 dark:border-white/10 dark:bg-black/40 dark:text-zinc-100 dark:shadow-[0_0_18px_rgba(15,23,42,0.8)]"
        }`}
    >
      {/* Petit logo horloge */}
